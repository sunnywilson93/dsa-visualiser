...
IMPORTANT: The executor MUST read the actual data files to enumerate ALL dynamic routes:
- Read `src/data/concepts.ts` to get all JS concept IDs -> generate `/concepts/{id}` routes
- Read `src/data/algorithmConcepts.ts` to get DSA concept IDs -> generate `/concepts/dsa/{id}` routes
- Read problem/category data to get category IDs -> generate `/{categoryId}` routes
...
- Include concept pages (`/[categoryId]/[problemId]/concept`) if they exist
