...
<task type="auto">
  ...
  <action>
Add the missing opacity variants to the @theme block in globals.css:

1. Find the @theme block section with color opacity variants (around lines 96-156)
2. Add missing variants after existing ones in each color's section:
...
Verify emerald and amber already have complete 15, 20, 30, 40 variants (they do per research).
  </action>
  <verify>
...
- bg uses 15% opacity, border uses 40% opacity (matching current inline patterns)
  ...
  <verify>
Run: `npx tsc --noEmit src/constants/levelInfo.ts`
Should complete with no errors. Also verify the file exists and exports are correct:
`grep -E "export (type|interface|const)" src/constants/levelInfo.ts`
  </verify>
  <done>
