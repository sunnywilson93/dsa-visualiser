...
<task type="auto">
  ...
  <action>
First, run grep to find all unique phase names used across the codebase:
```bash
grep -rh "getPhaseColor\|case '" src/components/Concepts/ | grep -oE "'[A-Za-z ]+'" | sort | uniq
```

Then create src/constants/phaseColors.ts with all discovered phases. Based on research, known phases include:
