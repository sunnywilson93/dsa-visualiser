...
<task type="auto">
  ...
  <action>
    ...
    4. Modify ConceptVizPageClient.tsx:
       ...
       - Add `<RelatedPatterns problemId={problemId} />` inside `<main>`, after the `conceptWrapper` div closing tag (line 87) and before the `practiceLink` Link component (line 89)
       - The component should be a sibling to both `conceptWrapper` and `practiceLink` within the `<main>` element
       - DOM structure after modification:
         ```tsx
         <main className={styles.main}>
           <div className={styles.conceptWrapper}>...</div>
           <RelatedPatterns problemId={problemId} />
           <Link className={styles.practiceLink}>...</Link>
         </main>
         ```
       - Flow: concept panel -> learn pattern -> practice code
  </action>
  <verify>
    1. `npm run build` passes without errors
