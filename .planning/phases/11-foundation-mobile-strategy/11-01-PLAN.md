...
4. Verify no JS viewport detection patterns:
   - grep -r "window.innerWidth\|matchMedia" src/app/[categoryId]/[problemId]/ should return no matches (confirms CSS-only approach)
  </verify>
  <done>Practice page shows Monaco on desktop (768px+) and ReadOnlyCode on mobile (below 768px) using CSS-only show/hide with no JS viewport detection</done>
</task>
...
6. No JS viewport detection: grep -r "window.innerWidth\|matchMedia" src/app/[categoryId]/[problemId]/ returns empty
</verification>
