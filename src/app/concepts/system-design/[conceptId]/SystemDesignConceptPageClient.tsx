...
const visualizations: Record<string, React.ComponentType> = {
  'sd-radio-framework': dynamic(() => import('@/components/Concepts/SystemDesign/SystemDesignViz').then(m => ({ default: m.SystemDesignViz })), { ssr: false }),
  'sd-component-architecture': dynamic(() => import('@/components/Concepts/SystemDesign/SystemDesignViz').then(m => ({ default: m.SystemDesignViz })), { ssr: false }),
  'sd-data-fetching': dynamic(() => import('@/components/Concepts/SystemDesign/SystemDesignViz').then(m => ({ default: m.SystemDesignViz })), { ssr: false }),
  'sd-state-management': dynamic(() => import('@/components/Concepts/SystemDesign/SystemDesignViz').then(m => ({ default: m.SystemDesignViz })), { ssr: false }),
  'sd-real-time': dynamic(() => import('@/components/Concepts/SystemDesign/SystemDesignViz').then(m => ({ default: m.SystemDesignViz })), { ssr: false }),
  'sd-rendering-strategies': dynamic(() => import('@/components/Concepts/SystemDesign/SystemDesignViz').then(m => ({ default: m.SystemDesignViz })), { ssr: false }),
  'sd-performance-budget': dynamic(() => import('@/components/Concepts/SystemDesign/SystemDesignViz').then(m => ({ default: m.SystemDesignViz })), { ssr: false }),
  'sd-accessibility': dynamic(() => import('@/components/Concepts/SystemDesign/SystemDesignViz').then(m => ({ default: m.SystemDesignViz })), { ssr: false }),
  'sd-design-feed': dynamic(() => import('@/components/Concepts/SystemDesign/SystemDesignViz').then(m => ({ default: m.SystemDesignViz })), { ssr: false }),
  'sd-design-autocomplete': dynamic(() => import('@/components/Concepts/SystemDesign/SystemDesignViz').then(m => ({ default: m.SystemDesignViz })), { ssr: false }),
}

export default function SystemDesignConceptPageClient(): JSX.Element {
  const params = useParams()
  const conceptId = params.conceptId as string

  const concept = conceptId ? getSystemDesignConceptById(conceptId) : undefined
    ...
    return (
      <PageLayout variant="content">
        <div className="flex flex-1 flex-col items-center justify-center gap-[var(--spacing-lg)] text-text-muted">
          ...
          <Link href="/concepts/system-design" className="text-[color:var(--color-brand-primary)]">Back to System Design Concepts</Link>
        </div>
      </PageLayout>
    )
  }
  ...
  return (
    <PageLayout
      ...
      breadcrumbs={[
        ...
        { label: 'System Design', path: '/concepts/system-design' },
        { label: concept.title },
      ]}
    >
        {/* Header */}
        ...
        {(() => {
          ...
          const relatedConcepts = relatedIds
            .map(id => getSystemDesignConceptById(id))
            .filter((c): c is NonNullable<ReturnType<typeof getSystemDesignConceptById>> => c !== undefined)
            ...
            <section className="mb-10">
              ...
              <div className="grid grid-cols-auto-sm gap-[var(--spacing-lg)]">
                {relatedConcepts.map((related) => (
                  <Link
                    key={related.id}
                    href={`/concepts/system-design/${related.id}`}
        ...
        <footer className="mt-[var(--spacing-2xl)] border-t border-[var(--color-border-card)] pt-[var(--spacing-xl)] text-center">
          ...
          >
            Browse all {systemDesignConcepts.length} System Design concepts
          </Link>
        </footer>
    </PageLayout>
  )
