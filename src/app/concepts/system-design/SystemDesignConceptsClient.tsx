...
export default function SystemDesignConceptsClient(): JSX.Element {
  const search = usePageSearch('system-design')
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-bg-page from-0% to-bg-page-secondary to-100%">
      ...
      <main className="flex-1 p-8 container-default mx-auto w-full max-md:p-6">
        <header className="text-center py-4 pb-8">
          ...
          <p className="text-text-secondary text-md m-0 leading-relaxed max-md:text-base">
            Master frontend system design through interactive visualizations. Learn frameworks, patterns, and real-world case studies.
          </p>
        </header>
        ...
        {hasActiveFilters ? (
          <section className="mb-12">
            ...
            <div className="bg-bg-page-secondary/50 border border-brand-primary-20 rounded-xl p-2">
              <SearchResultsList
                ...
                emptyMessage="No system design concepts match your search criteria"
              />
            </div>
          </section>
        ) : (
          <>
            {systemDesignCategories.map((category) => {
              const categoryConcepts = systemDesignConcepts.filter(c => c.category === category.id)
                ...
                <section key={category.id} className="mb-12">
                  ...
                  <CardCarousel itemCount={categoryConcepts.length}>
                    {categoryConcepts.map((concept, index) => (
                      <Card
                        key={concept.id}
                        href={`/concepts/system-design/${concept.id}`}
                        title={concept.title}
      ...
      <footer className="text-center p-8 text-text-muted text-base max-md:p-6">
        <p className="m-0 mb-2">
          {systemDesignConcepts.length} concepts across {systemDesignCategories.length} topics
        </p>
        <Link
          href="/concepts/react"
