...
export default function TSConceptsClient(): JSX.Element {
  const search = usePageSearch('ts')
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-bg-page from-0% to-bg-page-secondary to-100%">
      ...
      <main className="flex-1 p-8 container-default mx-auto w-full max-md:p-6">
        <header className="text-center py-4 pb-8">
          ...
          <p className="text-text-secondary text-md m-0 leading-relaxed max-md:text-base">
            Master TypeScript through interactive visualizations. Explore type narrowing, generics, utility types, and advanced patterns.
          </p>
        </header>

        <PageSearchControls
          search={search}
          placeholder="Search TypeScript concepts..."
        />
          ...
          <section className="mb-12">
            ...
            <div className="bg-bg-page-secondary/50 border border-brand-primary-20 rounded-xl p-2">
              <SearchResultsList
                ...
                emptyMessage="No TypeScript concepts match your search criteria"
              />
            </div>
          </section>
        ) : (
          <>
            {tsConceptCategories.map((category) => {
              const categoryConcepts = tsConcepts.filter(c => c.category === category.id)
      ...
      <footer className="text-center p-8 text-text-muted text-base max-md:p-6">
        <p className="m-0 mb-2">
          {tsConcepts.length} concepts across {tsConceptCategories.length} topics
        </p>
        <Link
          href="/concepts/react"
