...
// Queue = BFS = go WIDE first`,
...

  {
    ...
    description: 'A heap is a complete binary tree with a strict parent-child ordering rule. In a min-heap, parents are always smaller than children; in a max-heap, parents are always larger. This lets you repeatedly get the "best next" item in O(1) and update the heap in O(log n).',
    shortDescription: 'Fast min/max access with efficient updates',
    keyPoints: [
...
heap.extractMin() // 1
heap.extractMin() // 2`,
        explanation: 'Insert and extract are each O(log n); peek is O(1).'
      },
      {
        title: 'Top K Frequent with Heap Concept',
        code: `// Count frequencies first
const freq = new Map()
...
// If we only need k elements, keep a small heap and drop low frequency items quickly`,
        explanation: 'Heaps are useful when you only need the top-k frequencies, not full sorted order.'
      },
      {
        title: 'Merge K Sorted Lists Idea',
        code: `// Push each list head into a min-heap
...
explainMergeKSortedLists()`,
        explanation: 'This pattern shows why heaps excel at repeatedly selecting the current global minimum from many sorted sources.'
      },
    ],
    complexity: {
      average: {
    ...
    learningPath: [
      {
        ...
        problemIds: ['top-k-frequent', 'kth-largest-element'],
      },
      {
        stage: 'Multiple Sources',
        problemIds: ['merge-k-sorted-lists', 'meeting-rooms-ii'],
      },
      {
...
dfs([[1, 2], [2], [3], []], 0)  // [0, 1, 2, 3]`,
...

  {
    ...
    shortDescription: 'Fast prefix lookup and word-based search',
    keyPoints: [
...
console.log(trie.startsWith('ca')) // true`,
        explanation: 'Each character follows an edge in the tree. Shared prefixes reuse nodes and keep each operation linear in word length.'
      },
      {
        title: 'Top 3 Suggestions by Prefix',
        code: `class SuggestionTrieNode {
  constructor() {
    this.children = {}
    this.products = []
  }
...
console.log(suggester.getSuggestions('ba'))`,
        explanation: 'Storing limited suggestions at each node avoids scanning all words for every typed character.'
      },
    ],
    complexity: {
      average: {
    ...
    learningPath: [
      {
        stage: 'Prefix Foundations',
        problemIds: ['implement-trie-prefix-tree', 'design-add-and-search-words-data-structure'],
      },
      {
        stage: 'Dictionary Querying',
        problemIds: ['replace-words', 'search-suggestions-system', 'stream-of-characters'],
      },
      {
        stage: 'Trie + Search',
        problemIds: ['magic-dictionary', 'word-search-ii'],
      },
      {
        stage: 'Advanced Applications',
        problemIds: ['short-encoding-of-words', 'maximum-xor-of-two-numbers-trie', 'longest-word-in-dictionary'],
      },
    ],
  },
